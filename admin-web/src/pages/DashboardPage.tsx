...
import { Card, CardHeader, EmptyRow, ErrorBox, SkeletonCard, SkeletonTable, Table, Td } from "../components/ui";
      ...
      <Card>
        ...
        >
          {recent.loading && !recent.data ? (
            ...
            recent.data?.map((r) => (
              <tr key={r.id}>
                <Td className="font-medium text-slate-800">{r.task_type}</Td>
                ...
                <Td className="text-xs text-slate-500">{(r.duration_ms / 1000).toFixed(1)}s</Td>
                <Td className="text-xs text-slate-500">{formatTime(r.finished_at)}</Td>
              </tr>
            ))
          )}
        </Table>
