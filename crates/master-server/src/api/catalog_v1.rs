...
use crate::store::catalog_v1::{
    get_import_run, get_import_run_outcome_counts, get_or_create_source, list_import_run_items,
    list_import_run_items_export_chunk, list_import_runs, list_quarantined_records, list_sources,
    CatalogSourceRow, EditionDetail, ImportRunItemsPage, ImportRunRow, QuarantinedRecordRow,
};
...
pub async fn get_stats(
    State(state): State<AppState>,
    _auth: AuthenticatedUser,
) -> AppResult<Json<crate::catalog::stats::CatalogStatsResponse>> {
    Ok(Json(crate::catalog::stats::read_stats(&state).await?))
}
