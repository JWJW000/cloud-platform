...
-- 常量默认值由 PostgreSQL 作为表元数据保存，不重写 300 万+历史行。
-- -infinity 表示“迁移前已拥有”；新增版本仍记录真实的拥有时间。
ALTER TABLE editions
    ADD COLUMN IF NOT EXISTS owned_at TIMESTAMPTZ DEFAULT '-infinity'::timestamptz;
ALTER TABLE editions ALTER COLUMN owned_at SET DEFAULT now();

-- 后续步骤都复用这份精确候选集，避免反复全表扫描 editions。
CREATE TEMP TABLE migration_0020_unowned (
...
INSERT INTO migration_0020_unowned (edition_id)
SELECT c.edition_id
...
SELECT 'catalog.ownership_changed', 'edition', u.edition_id, '{}'::jsonb, '待同步'
FROM migration_0020_unowned u;
...
WITH unowned AS (
    ...
    FROM migration_0020_unowned candidate
    JOIN editions e ON e.id = candidate.edition_id
)
INSERT INTO books (
...
FROM migration_0020_unowned candidate
WHERE at.edition_id = candidate.edition_id
...
JOIN migration_0020_unowned candidate ON candidate.edition_id = at.edition_id
ON CONFLICT (id) DO UPDATE SET
...
JOIN migration_0020_unowned candidate ON candidate.edition_id = b.catalog_edition_id
WHERE bt.format = 'pdf'
ON CONFLICT (batch_id, book_id) DO UPDATE SET display_status = EXCLUDED.display_status;

-- 出版社统计只计算已拥有版本；候选版本仍可用于任务搜索，但不进入我的总库数字。
WITH affected_publishers AS (
    SELECT DISTINCT e.publisher_id
    FROM migration_0020_unowned candidate
    JOIN editions e ON e.id = candidate.edition_id
    ...
    SELECT p.id AS publisher_id,
           ...
           count(DISTINCT CASE WHEN lf.verify_status = '有效' AND h.meets_strategy THEN e.id END) AS editions_with_files_c
    FROM affected_publishers affected
    JOIN publishers p ON p.id = affected.publisher_id
    LEFT JOIN editions e ON e.publisher_id = p.id AND e.owned_at IS NOT NULL
